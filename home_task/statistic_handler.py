...
from models import JobPostingStatistics
...
class StatisticHandler:
    ...
    def processing_statistics(
        self, job_id: str, country_code: str | None, threshold: int, records: list
    ) -> None:

        min_threshold, max_threshold = self._get_min_max_thresholds(
            job_id, country_code
        )
        aggregated_data = self._create_statistic(
            job_id, min_threshold, max_threshold, country_code
        )
    ...
    def _get_unique_pair(self) -> list[JobPosting]:
        query = (
            ...
            .with_entities(JobPosting.country_code, JobPosting.standard_job_id)
            .filter(JobPosting.country_code.isnot(None))
            .distinct()
            .order_by(JobPosting.country_code)
        )
    ...
    def _get_unique_jobs(self) -> list[JobPosting]:
        query = (
            self.session.query(JobPosting)
            .with_entities(JobPosting.standard_job_id)
            .distinct()
        )
        return query.all()

    def _get_min_max_thresholds(
        self, job_id: str, country_code: str = None
    ) -> Tuple[Decimal, Decimal]:
        query = self.session.query(
            func.percentile_cont(DEFAULT_MIN_PERCENTILE)
            .within_group(JobPosting.days_to_hire)
            .label("p10"),
            func.percentile_cont(DEFAULT_MAX_PERCENTILE)
            .within_group(JobPosting.days_to_hire)
            .label("p90"),
        ).filter(
            JobPosting.standard_job_id == job_id, JobPosting.days_to_hire.isnot(None)
        )
        ...
        min_threshold = Decimal(percentiles.p10).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP
        )
        max_threshold = Decimal(percentiles.p90).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP
        )
    ...
    @staticmethod
    def convert_record(
        job_id: str, country_code: str | None, aggregated_data: Row
    ) -> dict:
        average_days = Decimal(aggregated_data.average_days).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP
        )
        return {
        ...
        for i in range(0, len(records), self.batch_size):
            ...
            conflict_stmt = stmt.on_conflict_do_update(
                index_elements=[
                    ...
                    func.coalesce(
                        JobPostingStatistics.__table__.c.country_code,
                        literal("__NULL__"),
                    ),
                ],
                set_={
    ...
    def get_statistic(self, job_id: str, country_code: str | None) -> Row | None:
        query = self.session.query(JobPostingStatistics).filter(
            JobPostingStatistics.standard_job_id == job_id
        )
